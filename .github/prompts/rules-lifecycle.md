...
指定された対象の `*-rules.md` と、その派生成果物（instruction / sample）を
2 フェーズのレビューサイクルで完成させてください。

Phase 1 では別の `*-input.md` は作成せず、Agent が `*-rules.md` の雛形を作り、
人がその中の `_TODO_:` / `_UNDECIDED_:` / `_ASSUMPTION_:` を追記・更新して完成させます。

## 対象の決定

- 開いているファイルが `*-rules.md` の場合 → そのファイルを Phase 1 の対象とする
- 開いているファイルが `*-instruction.md` / `*-sample.md` の場合 → 対応する `*-rules.md` を特定し Phase 1 から開始
...
1. **① agent draft** — `upsert-rules` Skill で `*-rules.md` の雛形を作成/更新し、必要箇所に `_TODO_:` / `_UNDECIDED_:` / `_ASSUMPTION_:` を入れて lint 検証する
2. **② human modify** — ドラフトの要約と改善候補を提示し、人が `*-rules.md` 本文中のラベル付き項目を追記・更新する
