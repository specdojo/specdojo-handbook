...
description: '`docs/ja/specdojo/guidelines/docs-contents-guide.md` を正として、対象となる rulebook / instruction / sample を 1 件ずつ独立に作成・更新し、最後に結果を集計する Skill です。'
---

# SKILL: upsert-all-rulebooks-instructions-samples

`docs/ja/specdojo/guidelines/docs-contents-guide.md` の「推奨ファイル名」を正として、
対象となる rulebook / instruction / sample を **1 件ずつ独立に** 作成・更新し、最後に結果を集計する Skill です。

この Skill の責務は **対象抽出・順序固定・個別実行・集計** です。
**各対象の内容判断と更新は、必ず対象ごとに独立して行い、1 件ずつ実行した場合と同じ厳密さを保つこと。**
...
- 一括 1 件処理: `@file:.github/skills/upsert-rulebook-instruction-sample/SKILL.md`
- 個別ステップの詳細（rulebook / instruction / sample）は `upsert-rulebook-instruction-sample` が参照する各 Skill 定義を正とする。
...
- 1 件ごとの処理厳密さは、rulebook / instruction / sample を対象単位で個別に実行した場合と同等でなければならない。
...
3. 各対象について、以下を **独立に** 実行する
   ...
   3. `upsert-rulebook-instruction-sample` をその対象のみに対して実行する
   4. 対象ごとの結果をその場で確定して記録する
...
- rulebook / instruction / sample は、その対象について **個別に結果を確定してから** 次の対象へ進む。
