...
description: '`docs/ja/handbook/guidelines/docs-contents-guide.md` を正として、対象となる instruction / sample を一括で作成・更新する Skill です。'
---

# SKILL: upsert-all-instructions-samples

`docs/ja/handbook/guidelines/docs-contents-guide.md` の「推奨ファイル名」を正として、
対象となる instruction / sample を一括で作成・更新する Skill です。
...
2. 対象を名前順で固定する（再現性確保）
3. 各対象について `rulebook-to-instruction` を実行する
4. 各対象について `rulebook-to-sample` を実行する
5. 途中失敗があっても、残り対象の処理を継続する
6. 最後に `npm run -s lint:md` を実行する
