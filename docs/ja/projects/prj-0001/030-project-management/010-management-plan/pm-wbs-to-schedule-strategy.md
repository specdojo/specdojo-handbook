...
| 管理単位   | Schedule は `schedule_group` 単位で管理する                          |
...
一方、`schedule_group` はスケジュールの管理単位を表します。

例:

```yaml
schedule_group: launch
schedule_group_code: LCH
```

```yaml
schedule_group: release-001
schedule_group_code: REL001
```

## 4. Schedule ファイル分割ルール

Schedule ファイル名は、`schedule_group` と `schedule_level` が分かる形式にします。

### 4.1. Milestones Schedule

```text
sch-<schedule-group>-milestones.yaml
```
...
```text
sch-<schedule-group>-<domain>.yaml
```
...
```text
sch-<schedule-group>-<domain>-<container>.yaml
```
...
```text
sch-<schedule-group>-cross-domain.yaml
```
...
```text
sch-<schedule-group>-<domain>-cross-container.yaml
```
...
| 要素      | ルール                                                        |
| --------- | ------------------------------------------------------------- |
| `GROUP`   | `schedule_group_code` を使用する                              |
| `SUBJECT` | `schedule_group` 内で一意に作業対象を識別できるキーを使用する |
| `NNN`     | 3 桁、`010` 刻みで採番する                                    |
...
| `schedule_group`      | 必須     | Schedule の管理単位。例: `launch`, `release-001`                                  |
| `schedule_group_code` | 必須     | ID に使用する短縮コード。例: `LCH`, `REL001`                                      |
...
| 項目            | 必須 | 記述ルール                                            |
| --------------- | ---- | ----------------------------------------------------- |
| `id`            | 必須 | `T-<GROUP>-<SUBJECT>-<NNN>` 形式を基本とする          |
...
| 項目         | 必須 | 記述ルール                                            |
| ------------ | ---- | ----------------------------------------------------- |
| `id`         | 必須 | `M-<GROUP>-<SUBJECT>-<NNN>` 形式を基本とする          |
...
3. `schedule_group` と `schedule_group_code` を決める
4. `schedule_level` を決める
5. `1 WBS Item = 1〜3 Task` を目安に Task へ展開する
6. Task の `id` を `T-<GROUP>-<SUBJECT>-<NNN>` 形式で採番する
...
- `schedule_level` と `schedule_group` を混同する
- `SUBJECT` が曖昧で、同じ `schedule_group` 内で作業対象を識別できない
...
- `schedule_group` と `schedule_group_code` は、一度使用したら原則として変更しません。
...
## Appendix A. Schedule Group と Schedule ファイルの対応定義

本付録は、Schedule の管理単位と Schedule 定義ファイルの対応を定義します。
`schedule_group` の追加に応じて更新します。

### A.1. Schedule Group

| schedule_group | schedule_group_code | 説明                                                           |
| -------------- | ------------------- | -------------------------------------------------------------- |
| `launch`       | `LCH`               | 立ち上げ時に必要なプロジェクト定義・管理計画を整備する管理単位 |

### A.2. Schedule ファイル

| schedule_group | schedule_level | 対象                                       | Schedule ファイル |
| -------------- | -------------- | ------------------------------------------ | ----------------- |
...
### B.2. domain: `project-definition`, schedule_group: `launch`
