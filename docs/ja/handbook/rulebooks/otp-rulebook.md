...
`otp-<term>` は、切替後の運用安定化と初期対応体制の対象別詳細を定義する個別SSOTとする。

- 共通方針（期間・SLA・優先度分類・クローズ基準）は `otp-index` を正とする
...
- `otp-<term>` は、領域/拠点/システム群/運用ドメイン単位で作成する
- `otp-<term>` の判断体系は `otp-index` と矛盾させない
- **「関連ドキュメント」セクションは必須**（分冊が無い場合でも `（本書のみ）` を明記して空欄にしない）

## 2. 位置づけ

`otp-<term>` と他ドキュメントの関係を示します。必要に応じて Mermaid による図を使用してよい。
...
- `otp-index`：運用切替の入口（共通方針、期間/SLA、クローズ基準）
- **`otp-<term>`：運用切替の対象別詳細（監視設定/運用手順/窓口）**

## 3. ファイル命名・ID規則

- `id` は `otp-<term>`（kebab-case。例: `otp-hypercare` / `otp-support-desk`）。
- ファイル名は `otp-020-運用切替計画-<term>.md` 等、プロジェクト内で一意になるように命名する。
...
- 対象範囲（どの運用領域/チーム/サービスが対象か）を明記する
...
`otp-<term>` では、`otp-index` への導線を必ず記載する。
