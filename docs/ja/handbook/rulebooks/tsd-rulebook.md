...
Frontmatter は `docs/handbook/shared/schemas/deliverables-frontmatter.schema.yaml` の制約に従います。
...
| based_on   | 根拠となる仕様ID（DEC/非機能/設計など） | 任意 |
...
> - Frontmatter は `docs/handbook/shared/schemas/deliverables-frontmatter.schema.yaml` に従い、`id`/`type`/`title`/`status` を必ず含め、`type` は `architecture` 固定にしてください。
