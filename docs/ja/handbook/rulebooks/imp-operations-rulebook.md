...
Impact Analysis (Operations) Rulebook
...
- 1 ドキュメントで複数の運用領域を扱ってよいが、影響詳細は運用対象単位で分割して管理する。
- データ変更、I/F 変更、テスト計画への波及がある場合は、本書で概要と判断を記載し、詳細は各ドキュメントへ委譲する。
...
| 4   | 影響詳細一覧   | ○    | 影響対象運用ごとの変更要否・影響度を記録する |
...
- サマリで使用した評価観点（例: 監視体制、障害初動、運用負荷、統制・監査影響）を先に示す。
...
| カラム       | 内容                                               |
| ------------ | -------------------------------------------------- |
| 影響対象運用 | 影響を受ける運用対象（監視、手順、体制、統制など） |
| 変更要否     | `要` / `否`                                        |
| 影響度       | `高` / `中` / `低`                                 |
| 対応方針     | 実施・代替・見送りなどの方針                       |
| 判断根拠     | 判定理由の要点                                     |
...
- 参照: `../samples/imp-operations-sample.md`

## 9. 生成 AI への指示テンプレート

- 参照: `../instructions/imp-operations-instruction.md`
