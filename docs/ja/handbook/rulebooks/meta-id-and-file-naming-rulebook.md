...
ただし運用上の既定として、**ファイル名はドキュメントIDと一致**させることを推奨する。

ID とファイル名を一致させない場合は、可読性・対外配布などの明確な理由を持つこと。
...
- 形式：`<prefix>-<term>`
  - 種類がある場合は、`<prefix>-<kind>-<term>`

- `<term>`も含め`kebab-case`: 英小文字 + 数字 + `-`
- **名詞句**で表現（動詞開始は禁止）
...
- **既定推奨は `ファイル名 = ドキュメントID`**
  - 例：`mtp-index` → `mtp-index.md`
- 機械処理は引き続き ID を正とし、ファイル名は機械判別の根拠にしない
- 日本語ファイル名は例外として許可する
  - 例：対外配布、業務部門レビュー、可読性最優先の成果物
- 日本語ファイル名の場合は、
  - 系列入口（`-index`）のファイル名は **suffix無し**を推奨（例：`移行計画.md`）
  - 個別（`-<term>`）は **`<系列名>-<対象名>.md`** を推奨（例：`移行テスト計画-カットオーバーリハーサル.md`）
- 例外時も frontmatter の ID を正として保持する
...
- ファイル名は **人間向け**だが、既定は **ドキュメントIDと同名**を推奨する
- 日本語ファイル名を使用してよい（例外運用）
...
| ドキュメントID          | 既定のファイル名例         | 例外時のファイル名例                         |
| ----------------------- | -------------------------- | -------------------------------------------- |
| `uts-index`             | `uts-index.md`             | `単体テスト仕様.md`                          |
| `uts-inventory`         | `uts-inventory.md`         | `単体テスト仕様-在庫管理.md`                 |
| `bdd-common`            | `bdd-common.md`            | `業務データ辞書-共通.md`                     |
| `tsp-index`             | `tsp-index.md`             | `テスト戦略・方針.md`                        |
...
- 既定（ID同名運用）では、IDをそのままファイル名として使用する
  - 例：`xxx-index` → `xxx-index.md`
  - 例：`xxx-common` → `xxx-common.md`
- 例外（日本語ファイル名運用）では次を推奨する
...
| 運用モード               | `-index`                            | `-common`                               |
| ------------------------ | ----------------------------------- | --------------------------------------- |
| 既定（ID同名）           | `xxx-index.md`                      | `xxx-common.md`                         |
| 例外（日本語ファイル名） | **suffix無し**（例：`移行計画.md`） | `-共通`（例：`業務データ辞書-共通.md`） |

> 入口は `-index` とIDで固定する。既定は ID 同名ファイル名とし、必要時のみ人間向け名称へ寄せる。
...
- ただし運用の既定としては `ファイル名 = ドキュメントID` を推奨する
...
- 新規作成時は原則として `ファイル名 = ドキュメントID` を採用する
- 既存文書は段階的に移行する（更新時・改版時に同名化を検討）
- 例外（日本語ファイル名）を採用する場合は、可読性や対外共有などの理由を明確化する
