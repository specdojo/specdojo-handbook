...
本ドキュメントは、対象別データ移行設計ドキュメント（`dmd-<term>`）を統一形式で記述するためのルールです。
旧→新のデータ移行を再現可能に定義し、品質（正確性・完全性）を担保するための内容を定義します。

## 1. 全体方針

`dmd-<term>` は、移行対象データの仕様、変換、実行、検証を **再現可能** な粒度で整理し、品質を担保するための個別SSOTとする。

- 共通方針（抽出方式の扱い、run_id規約、冪等性、検証の種類）は `dmd-index` を正とする
...
- `dmd-<term>` はデータ領域（例：受注/請求/在庫）やデータセット単位で作成する
- `dmd-<term>` の判断体系は `dmd-index` と矛盾させない
- **「関連ドキュメント」セクションは必須**（最低でも `dmd-index` への導線を記載する）

## 2. 位置づけ

`dmd-<term>` と他ドキュメントの関係を示します。必要に応じて Mermaid による図を使用してよい。

- `mip-index`：移行の合意・判断のSSOT（成功条件・カットオーバー骨格を含む）
- `dmd-index`：データ移行の入口（共通方針、run_id規約、検証の共通方針）
- **`dmd-<term>`：対象別データ移行の詳細設計（マッピング/変換/検証）**
...
- ファイル名は `dmd-020-データ移行設計-<term>.md` 等、プロジェクト内で一意になるように命名する。
...
生成する `dmd-<term>` 本文の見出しは **## 10. 関連ドキュメント（必須）**
...
| `dmd-index` と矛盾する共通方針（run_id/冪等性/検証種別）を記載する | ドキュメント間で判断基準が分裂するため                         |
