...
ただし、すべてのプロジェクトで全項目を常時管理する必要はない。
プロジェクト規模、外部関係者の有無、レビュー・承認の厳密さに応じて、最小・標準・拡張の 3 段階から必要な厚みを選ぶ。
...
| メンバー              | 実行主体の nickname、表示名、種別、対応ロール | `pm-members.yaml`                                                                    |
...
People / Organization 定義は、次の 3 段階で運用する。

| レベル | 適用場面                                 | 管理するもの                                              | 代表ファイル                                                 |
| ------ | ---------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------ |
| 最小   | 個人・小規模・AI 実行をまず回したい場合  | ロール、member、Schedule owner                            | `prj-organization.md`, `pm-members.yaml`, `sch-track-*.yaml`    |
| 標準   | 複数人・レビュー・承認がある場合         | 最小 + RACI                                               | 最小 + `pm-raci.md`                                          |
| 拡張   | OSS 公開・外部協力・監査・調整がある場合 | 標準 + stakeholder register、関与方針、コミュニケーション | 標準 + `prj-stakeholder-register.md`, communication 関連文書 |

### 2.1. 最小運用

最小運用では、`sch` のタスクを誰が実行できるかに必要な情報だけを管理する。
prj-0001 のような個人・小規模プロジェクトでは、通常はこのレベルから開始してよい。

最小運用で必要な項目:

| 定義         | 最小項目                            | 目的                                  |
| ------------ | ----------------------------------- | ------------------------------------- |
| ロール定義   | ロールコード、正式名称、主な責務    | `tasks[].owner` に使う値を固定する    |
| メンバー定義 | nickname、display_name、owner、type | `specdojo exec --by` の候補を定義する |
| Schedule     | Task ID、WBS ID、`owner`、依存関係  | 実行可能なタスクに展開する            |

最小運用では、RACI、影響度/関心度分析、エンゲージメント方針、コミュニケーション要件は必須ではない。
必要になった時点で標準運用または拡張運用へ進める。

### 2.2. 標準運用

標準運用では、レビュー・承認・相談先を明確にするために RACI を追加する。
複数人で作業する場合、または agent が草案作成し人間が承認する場合は、このレベルを推奨する。
...
最小運用では Organization Role、Member、Schedule Assignment を中心に扱い、RACI と Stakeholder は必要時に追加する。
...
| 層                  | 識別子                               | 目的                     | 安定性                         |
| ------------------- | ------------------------------------ | ------------------------ | ------------------------------ |
| Stakeholder         | `STH-PROJECT-OWNER` など             | 利害関係・合意対象の識別 | 中程度。関係者追加で増える     |
...
ステークホルダー ID は、拡張運用で stakeholder register を管理する場合に使用する。
...
### 7.1. `prj-stakeholder-register.md`

ステークホルダー登録簿は、拡張運用で使用する。
関係者の利害、期待、懸念、必要な合意、関与方針を管理する。
...
| `--by`  | タスクを claim / 実行する主体 | `po`, `ba-agent`        | `pm-members.yaml` / 実行イベント |
...
ある Task の `owner` が `BA` の場合、実行候補は `pm-members.yaml` の `owner: BA` を持つ member である。
...
- `pm-members.yaml` の `members[].owner` は、`null` または組織定義に存在するロールコードでなければならない。
...
prj-0001 のような小規模プロジェクトでは、まず次の構成だけで運用してよい。
