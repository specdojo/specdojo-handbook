...
- `SPECDOJO_OWNER` 環境変数または `--owner` オプションで実行時に役割を宣言する（例: `--owner ARC`）。
...
`--by` に指定するニックネームと担当者情報（氏名・メールアドレス等）は
[members.yaml](./members.yaml) で管理します。
ニックネームは一度イベントログに記録されると **変更不可** です（履歴の一貫性を保つため）。

| ケース                                           | 操作                                                                           |
| ------------------------------------------------ | ------------------------------------------------------------------------------ |
| 自分の役割ラベルに割り当てられたタスクを着手する | `specdojo exec scheduler --by <nickname> --owner <PO\|BA\|ARC\|QE>`            |
| 相手の役割ラベルのタスクを代理で着手する         | `--allow-owner-mismatch` を付加し、`--msg` に代理理由を記録する                |
| 担当変更を永続化したい場合                       | schedule/WBS の `owner` フィールドを更新し、`specdojo exec build` を再実行する |
