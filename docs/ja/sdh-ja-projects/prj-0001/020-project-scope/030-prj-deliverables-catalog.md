...
#### 2.4.3. レポート（030-reporting）

<!-- prettier-ignore -->
| name | 成果物名 | 概要 |
| --- | --- | --- |
| `progress-reports` | 進捗報告 | 定期的な進捗状況の報告 |
| `meeting-minutes` | 議事録 | 会議の決定事項・アクションを記録 |
...
| name | 成果物名 | 概要 |
| --- | --- | --- |
| `esil` | 外部システム IF 一覧 | 外部システムとの接続一覧と IF 概要 |
| `eapis` | 外部 API 仕様 | 外部 API のエンドポイント・パラメータ・応答を定義 |
| `efes` | 外部ファイル連携仕様 | 外部ファイル連携のレイアウト・形式・頻度を定義 |
| `ems` | 外部メッセージ仕様 | 外部メッセージの形式・プロトコル・処理を定義 |
...
| `tsl` | 技術スタック定義 | 採用技術・バージョン・選定理由を定義 |

## 6. システム設計

システム全体設計・横断ルール・画面仕様に関する成果物です。
...
| `itd` | 内部結合テスト設計 個別 | 個別の内部結合テスト設計・手順・期待結果 |
