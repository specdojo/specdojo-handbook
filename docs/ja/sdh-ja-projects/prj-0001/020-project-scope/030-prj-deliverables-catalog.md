...
`docs/ja/sdh-ja-projects/prj-0001`以下に配置するプロジェクト運営・管理に関する成果物です。
...
## 2. エージェントカスタマイズ（.github/） `agent-customization`
